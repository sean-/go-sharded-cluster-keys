// Package key64 provides a reusable Encoder for shuffling bits
// of a 64-bit value: extracting a run of bits, reversing them,
// and prepending them into the high bits of a 64-bit word.
package key64
...
type Encoder interface {
	...
	Encode(v uint64) Value

	// Decode is the inverse of Encode, returning the original uint64.
	Decode(e Value) uint64

	// Prefix extracts the top size bits of e (the reversed segment).
	Prefix(e Value) uint64
...
func (e encoder) RightSize() int  { return 64 - e.offset - e.size }

// Encode implements Encoder.Encode
func (e encoder) Encode(v uint64) Value {
	...
	enc := (rev << (64 - e.size)) | (left << e.offset) | right
	return Value(enc)
}

// Decode implements Encoder.Decode
func (e encoder) Decode(val Value) uint64 {
	u := uint64(val)
	mask := uint64((1 << e.size) - 1)

	// 1) pull out and unreversed the top size bits
	rev := (u >> (64 - e.size)) & mask
	field := reverseBits(rev, e.size)

	// 2) split the rest
	left := (u >> e.offset) & ((1 << (64 - e.size - e.offset)) - 1)
...
func (e encoder) Prefix(val Value) uint64 {
	u := uint64(val)
	return (u >> (64 - e.size)) & ((1 << e.size) - 1)
}
